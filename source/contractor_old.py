...
class ContractorAgent(Agent):
    ...
    def step(self, scheduler, agent_keys, cases):
        method = "step"
        agent_keys = agent_keys[1:]  # Exclude contractor

        if not agent_keys:
            return  # No agents to consider

        if self.model.central_orchestration:
            # Central: no prior agent, just use transition probs from current prefix
            current_agent = None
            current_activity = None
        else:
            # Decentral: we rely on previous agent and activity
            current_agent = self.case.previous_agent
            if current_agent == -1:
                current_activity = None
            else:
        ...
        sorted_agent_keys = agent_keys  # Default fallback: keep order

        if not self.model.central_orchestration and current_agent != -1:
            # Autonomous mode: use transition probabilities conditioned on current agent and activity
            if current_agent in self.model.agent_transition_probabilities:
                if current_activity in self.model.agent_transition_probabilities[current_agent]:
                    current_probs_dict = self.model.agent_transition_probabilities[current_agent][current_activity]
                    # Build probability list for available agents
                    agent_probs = {
                        agent: current_probs_dict.get(agent, {}).get(next_activity, 0)
                        for agent in agent_keys
                    }
                    eligible_agents = [agent for agent, prob in agent_probs.items() if prob > 0]
                    if eligible_agents:
                        probs = [agent_probs[agent] for agent in eligible_agents]
                        sorted_agent_keys = random.choices(eligible_agents, weights=probs, k=len(eligible_agents))
        elif self.model.central_orchestration:
            # Central mode doesn't use per-agent transition probs
        ...
        for agent_key in sorted_agent_keys:
            ...
            if agent_key in scheduler._agents:
                if self.activity_performed:
                    ...
                    current_timestamp = self.get_current_timestamp(agent_key)
                    getattr(scheduler._agents[agent_key], method)(
                        last_possible_agent,
                        parallel_activity=False,
                        current_timestamp=current_timestamp,
                        perform_multitask=False
